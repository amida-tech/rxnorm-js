...
function queryfda(q, label, callback) {
    query(util.format("https://api.fda.gov/drug/%s.json", label), q, callback);
}
...
// expand openfda search to look at both the labels and the adverse reactions
// flag that is an indicator for searching label versus event (event is default)
// label is used to identify additional openfda data
function queryfdaName(medname, label=false, callback) {
    if (label === false){
      queryfda({
      ...
      queryfda({
          search: "openfda.generic_name:" + medname + "+brand_name:" + medname,
